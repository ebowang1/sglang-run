#!/usr/bin/env python3
"""
KV load 一键实验脚本（单进程自动编排，无需两个终端 / 手动 Enter）。
...
用法：
  python kv_profile_auto.py --tag run-001                 # 只 metrics（近100%命中）
  python kv_profile_auto.py --tag run-prof-001 --profile  # metrics + profile(前4条)
...
CHAT_URL = URL_BASE + "/v1/chat/completions"
...
def diff_metrics(before, after, prefix=16000, new_tokens=0):
    ...
    show("evicted_tokens_total")
    show("hicache_host_used_tokens")
    ...
    if ok_host and ok_load:
        # 逻辑 H2D 估算：5.14GB / 55040 ≈ 93.4KB/token（load_back 含主KV+indexer两组件）
        gb = d_load * 5.14 / 55040
        print(f"  => 确认 KV load 有效。逻辑 H2D ≈ {gb:.2f} GB "
              f"(Δload_back_tokens={d_load:.0f} × 5.14GB/55040)")
        ...
        # 实际命中率：命中前缀 token / (命中 + 新算)。新算总量≈ round_n 条 × new_tokens
        if new_tokens > 0:
...
def run_once(args, run_tag):
    ...
    if args.profile:
        print(f"\n[trace] 两台各看: find {args.output_dir} -name '{run_tag}*.trace.json.gz'")
    return ok



def main():
    ap = argparse.ArgumentParser(description="KV load 一键实验（metrics + 可选 profile）")
    ...
    ap.add_argument("--profile", action="store_true", help="是否抓 profile")
